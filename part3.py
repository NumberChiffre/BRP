...
from BRP.greedy.greedy_third import GreedyTraversalThird
...
if __name__ == '__main__':
    ...
    for idx in range(num_test_iterations):
        ...
        gt = GreedyTraversalThird(obj.graph, obj.bike_data)
        ...
        title = 'Figure 1: Demand Decrease and Number of Vehicles, Part III'
        plt.title(title)
        bar_width = 0.25
        ...
        plt.savefig(f'{ROOT_DIR}/results/results_{idx + 1}_partIII.png')
        plt.clf()
        print(
            f'vehicles needed for simulation #{idx + 1}: {len(min_traversals)}')
    df.to_csv(f'{ROOT_DIR}/results/results_partIII.csv')

    print('PART II Heuristic:')
    for idx in range(num_test_iterations):
        obj = GridGenerator(idx=idx + 1)
        obj.data_generator()
        obj.topology_generator()
